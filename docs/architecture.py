...
# crop the output automatically based on used image space
def trim(im, border=50):
    ...
    if bbox:
        x1, y1, x2, y2 = bbox
        return im.crop((x1-border, y1-border, x2+border, y2+border))

diag.dot.format = 'png'
im = Image.open(io.BytesIO(diag.dot.pipe()))
trim(im).save(output + '.png')
